...
// /*  AES_Encrypt Test   
int main () {  
    const char* key = "4C336B52396E5970586A54715A7756734366556749684F644561426D417A5062"; // 64 hex chars = 32 bytes (AES-256)
    const char* text = "Hello World, This is AES Encryption!";
    const char *iv = "5f12ab9f2cd7e48f736e0a5b98f1c27d"; // 32 hex chars = 16 bytes
    AES_type aesType = AES_256;
    size_t keyHexLen = 64;

    printf("\nOg text : \n%s\n", text);

    char* encryptedHexStr = AES_encrypt((const uint8_t*)text, strlen(text), key, keyHexLen, iv, aesType);
    printf("\nEncrypted Hex String : \n%s\n", encryptedHexStr);

    char* decryptedText = AES_decrypt(encryptedHexStr, key, keyHexLen, iv, aesType);
    printf("\nDecrypted Text : \n%s\n", decryptedText);

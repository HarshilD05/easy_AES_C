...
const uint8_t* sBoxGen (unsigned int seed) {  
	size_t itt = 0xFF;
...
const uint8_t* inverseSBoxGen (const uint8_t* sBox) {
    ...
    for (i = 0;i<=0xFF;++i) {
        coordinate = sBox[i];
        inverseSBox[coordinate] = i;
    }

    return (const uint8_t*)inverseSBox;
}
...
int main () {
	...
	const uint8_t* genSBox = sBoxGen(0U);

	printSBox(genSBox);

	const uint8_t* inverseSBox = inverseSBoxGen(sbox);
